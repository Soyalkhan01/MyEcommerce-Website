...
if os.path.exists(ORDERS_FILE):
    with open(ORDERS_FILE, "r", encoding="utf-8") as f:
        ...
        if orders:
            for order in orders:
                orders_col.update_one({"id": order["id"]}, {
    ...
    try:
        ...
        for field in ["age", "reviews", "rating"]:
            if field in data:
                try:
                    data[field] = int(
                        data[field]) if field != "rating" else float(data[field])
                except:
                    data[field] = 0
...
def preloader():
    ...
    """
    try:
        data = collection.find_one({"preloader": {"$exists": True}}, {
                                   "_id": 0, "preloader": 1})
        if not data:
...
def update_user(user_id):
    ...
    if "password" in update_data:
        update_data["password"] = generate_password_hash(
            update_data["password"], method="pbkdf2:sha256", salt_length=16)
...
def electronics():
    data = read_data()
    electronics = [e for e in data.get(
        "electronics", []) if not e.get("deleted", False)]
...
def permanent_delete_electronics(id):
    data = read_data()
    data["electronics"] = [e for e in data.get(
        "electronics", []) if e["id"] != id]
    write_data(data)
    return jsonify({"success": True})
